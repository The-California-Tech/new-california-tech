...
<script lang="ts">
  ...
  // $state + $effect here is deliberate, not an oversight. SSR renders the
  // server's timezone; the $effect below re-runs on the client to correct it to
  // the visitor's local date. A writable $derived would evaluate during
  // hydration and produce a server/client mismatch instead of a correction.
...
<a
  ...
  <div id="banner" class="max-w-4xl mx-auto overflow-hidden cursor-pointer hover:opacity-90 transition-opacity">
