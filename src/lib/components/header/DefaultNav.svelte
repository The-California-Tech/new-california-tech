...
<div class="flex items-center justify-between max-w-7xl mx-auto px-4 sm:px-8 h-full gap-4">
  ...
  <nav class="hidden lg:flex items-center gap-1" aria-label="Primary">
    {#each sections as section (section.href ?? section.name)}
      <a
        href={section.href}
  ...
  <details class="lg:hidden relative" role="navigation">
    <summary class="px-3 py-2 cursor-pointer list-none hover:bg-black/10 dark:hover:bg-white/10 rounded">Menu</summary>
    <div
      class="absolute top-full left-0 mt-2 bg-white dark:bg-black border-2 border-black dark:border-white rounded-lg shadow-lg min-w-[200px] z-50">
      {#each sections as section (section.href ?? section.name)}
        <a
          href={section.href}
  ...
  <div class="ml-auto flex items-center gap-2">
    ...
    <div class="flex items-center js-only-search">
      ...
      {:else}
        <button onclick={toggleSearch} class="btn group" aria-label="Open search">
...
<noscript>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-8 pb-2">
    <form
      ...
      method="GET"
      class="flex items-center border-2 border-black dark:border-white rounded overflow-hidden">
      <input type="search" name="q" placeholder="Search articles..." class="flex-1 px-3 py-2 bg-transparent" />
      <button type="submit" class="px-4 py-2 hover:bg-black/10 dark:hover:bg-white/10">Search</button>
    </form>
  </div>
</noscript>
