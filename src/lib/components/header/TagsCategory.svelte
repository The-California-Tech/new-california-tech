...
<script lang="ts">
  ...
  let { category, activeTag = $bindable() } = $props<{
  ...
  <details bind:open={expanded}>
    ...
    {#if expanded}
      <ul class="pl-2 pt-2 flex flex-col items-start gap-1">
        {#each category.tags as tag (tag.name)}
          <li>
            <a
              href="/?tag={tag.name}"
              class="text-base p-1 rounded hover:bg-black/10 dark:hover:bg-white/10 transition-colors {activeTag ===
              tag.name
                ? 'font-bold bg-black/15 dark:bg-white/15'
                : ''}"
