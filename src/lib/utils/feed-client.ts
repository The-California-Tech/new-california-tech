...
export function buildCountLoadMoreHref(options: BuildCountLoadMoreHrefOptions): string {
  ...
  const nextCount = options.nextCountHint ?? (options.shownCount ?? options.renderedCount) + options.batchSize;
...
export function observeInfiniteScroll(target: Element, onIntersect: () => void, rootMargin = '320px 0px'): () => void {
...
/**
 * REMOVED (Sep 2026): getActiveIssueDateFromRoot(), which measured
 * `.issue-section[data-issue-date]` offsets on every scroll frame to decide
 * which issue to rewrite the URL to. Both callers -- `/` and /issues/[date] --
 * dropped that behaviour, so this measured nothing.
 *
 * The `.issue-section[data-issue-date]` markup in index_posts.svelte is still
 * live; it drives the issue separators, not URL state.
 */
