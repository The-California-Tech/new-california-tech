...
async function backfill() {
  ...
  const needsUpdate = (files ?? []).filter(
    (f: { name: string; id: string }) => !known.has(`media/${f.name}`) && /\.\w+$/.test(f.name),
  );
  ...
  for (const file of needsUpdate) {
    ...
    const { error: upsertError } = await supabase.from('image_metadata').upsert(
      {
        bucket_id: 'media',
